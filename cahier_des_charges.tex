...
F05 & Catalogue et recherche & Filtres combinés (titre, auteur, prix, état). \\
...
\noindent Le système ne définit que \textbf{deux rôles techniques} (\texttt{admin} / \texttt{user}) : Administrateur et Vendeur (bénévole) partagent le même rôle \texttt{admin} (même type de compte, aucune distinction en base) ; Acheteur correspond au rôle \texttt{user}.

\begin{longtable}{@{}L{2.6cm}L{1.6cm}L{3.9cm}L{6cm}@{}}
\toprule
\textbf{Acteur (métier)} & \textbf{Rôle} & \textbf{Objectif} & \textbf{Actions autorisées} \\
\midrule
\endhead
Administrateur & \texttt{admin} & Qualité et bon fonctionnement des annonces & CRUD annonces, modération, gestion des comptes \\
Vendeur (bénévole) & \texttt{admin} & Vendre rapidement les livres du stock & Ajouter, fixer un prix, modifier, supprimer ses annonces \\
Acheteur & \texttt{user} & Trouver et acheter un livre & Rechercher, filtrer, consulter, acheter via PostFinance \\
\bottomrule
\caption{Acteurs du système}
\end{longtable}

\noindent\textbf{Gouvernance :} la commanditaire valide les livrables au fil des démonstrations ; Alexandre REY
...
SF-02 & Ajout d'un livre par scan ISBN, saisie manuelle en secours ; si OpenLibrary ne fournit aucune couverture (ou échoue), photo du livre prise via la caméra arrière du terminal utilisée comme image de secours & Livre reconnu ajouté en moins de 2 minutes ; seul un code-barres ISBN valide (préfixe 978/979 et chiffre de contrôle) est retenu, les autres codes-barres du livre étant ignorés. La lecture automatique dépend de l'API navigateur \texttt{BarcodeDetector}, absente de Safari et de Firefox : la saisie manuelle y est proposée explicitement. \\
...
SF-05 & Panier avec réservation temporaire du stock & Livre jamais vendu deux fois (un exemplaire vendu quitte définitivement le stock) ; panier abandonné libère le stock automatiquement, et l'ouverture d'un nouveau panier libère immédiatement le précédent ; le montant total enregistré est toujours égal à la somme des lignes majorée des frais de port, y compris lorsque les articles sont ajoutés simultanément. \\
...
SF-09 & Conformité RGPD / nLPD & Demande d'effacement anonymise effectivement les données personnelles ; refusée (409) tant qu'une commande de l'utilisateur n'a pas atteint un statut terminal (payée/expédiée en cours exclue), pour préserver l'obligation de conservation comptable. \\
SF-10 & Accessibilité de base & Parcours critiques utilisables au clavier. \\
SF-11 & Retrait d'un livre épuisé & Un administrateur peut retirer du catalogue un livre dont il ne reste aucun exemplaire dans aucun magasin ; l'opération est refusée tant qu'un exemplaire est en stock ou réservé, et les lignes de commande existantes sont préservées. \\
SF-12 & Messages d'erreur en français & Tout message d'erreur présenté à l'utilisateur final est rédigé en français, y compris les réponses d'erreur génériques de la plateforme (validation, indisponibilité) et les erreurs remontées par le prestataire de paiement. \\
\bottomrule
...
(soirées de 17h--20h, environ 3 par semaine), projet mené sur environ 3 mois en flux continu
de type \textbf{Kanban} plutôt qu'en sprints à durée fixe — un développeur solo en apprentissage
ne travaillant pas à intervalle régulier, ce découpage colle davantage à la réalité qu'un Scrum
classique —, aucun budget dédié.
...
\noindent\textbf{Planning :} flux continu structuré en cinq grandes phases (voir le détail et
les livrables de chacune dans le mémoire de projet) plutôt qu'en sprints calendaires figés :
fondations (environnement, CI/CD, modélisation BDD) ; cœur fonctionnel (authentification, ajout
de livre / scan ISBN, tunnel de paiement, panier et stock) ; fiabilisation et sécurité (durcissement,
mise en place de la CI) ; consolidation admin (suivi des commandes) ; documentation et
certification.
...
La recette est réalisée par démonstration à la commanditaire, à chaque étape marquante puis en fin de
...
SF-10 & Parcours critiques utilisables au clavier & Conforme avec réserve (lien d'évitement, focus visible et éléments interactifs natifs sur l'ensemble du site ; pas d'audit RGAA/WCAG externe ni de test avec lecteur d'écran réel) \\
SF-11 & Retrait d'un livre épuisé dans tous les magasins & Conforme \\
SF-12 & Messages d'erreur en français pour l'utilisateur final & Conforme avec réserve (le message technique du prestataire de paiement, en anglais, est conservé entre parenthèses après la phrase française, à des fins de support) \\
\bottomrule
