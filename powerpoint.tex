...
\begin{frame}{Plan de la présentation}
  \begin{columns}[T]
    ...
    \column{0.5\textwidth}
    \begin{exampleblock}{Le principe de cette présentation}
      \small Plutôt que de vous lister des fonctionnalités une par une, je vais suivre
      \textbf{un seul livre}, de son dépôt chez Caritas jusqu'à sa livraison, et vous
      montrer à chaque étape la brique technique qui le fait avancer.
    \end{exampleblock}
    \vspace{6pt}
    \begin{center}
    \begin{tikzpicture}[every node/.style={font=\tiny}, node distance=6mm]
      \node[tag] (a) {1. Scan};
      \node[tag,right=of a] (b) {2. Catalogue};
      \node[tag,right=of b] (c) {3. Achat};
      \draw[flow] (a)--(b); \draw[flow] (b)--(c);
      \node[tag,below=8mm of a,xshift=5mm] (d) {4. Livraison};
      \node[tag,right=of d] (e) {5. Clôture};
      \draw[flow] (c) -- ++(0,-0.3) -| (d);
      \draw[flow] (d)--(e);
    \end{tikzpicture}
    \end{center}
...
\begin{frame}{Mon parcours --- avant et pendant ce projet}
  \begin{columns}[T]
    ...
    \begin{block}{Étudiant alternant --- ESTIAM E3 DAD}
      \begin{itemize}\small
        \item \textbf{Mission 1 (5 mois) --- CERN}, la plus grande expérience
          scientifique d'Europe : développement low-code sur \textbf{Oracle APEX}
          (applications sur base Oracle on-premise), déploiement d'une application
          ETL open source nommée \textbf{Prefect}, orchestrée sur \textbf{Kubernetes}
        \item \textbf{Mission 2 (actuelle, 6\textsuperscript{e} mois) --- Qiminfo},
          ESN suisse : \textbf{Power Apps}, \textbf{Power Automate}, \textbf{SharePoint}
          --- et une transition en cours vers le développement \textbf{C\#/.NET (Razor)}
      \end{itemize}
    \end{block}
    \vspace{6pt}
    \begin{center}
    ...
    \begin{exampleblock}{Le lien avec Caribooks}
      \small D'un environnement low-code (Oracle APEX, Power Platform) à un projet
      \textbf{full-code} de bout en bout --- FastAPI, Next.js, MySQL --- Caribooks est
      l'endroit où j'ai pu pousser le développement \og pur code \fg{} le plus loin,
      juste avant de plonger dans le C\#/.NET.
    \end{exampleblock}
  \end{columns}
\end{frame}
...
\begin{frame}{Contexte du projet — Association Caritas}
  ...
  \begin{exampleblock}{\small Pourquoi j'ai choisi ce projet}
    \small Une association qui aide concrètement à la réinsertion, un besoin réel et
    mesurable (des cartons de livres qui dorment en réserve), et un périmètre complet
    à porter seul, du schéma de base de données jusqu'au serveur de production.
  \end{exampleblock}
\end{frame}
...
% ─────────────────────────────────────────
%  SLIDE 6 — STACK TECHNIQUE + ARCHITECTURE
% ─────────────────────────────────────────
\begin{frame}{Stack technique --- Les choix, et pourquoi je les ai faits}
  \begin{block}{Justifications des choix}
    \begin{itemize}\small
      ...
      \item \tech{MySQL} + \tech{SQLAlchemy} : relationnel éprouvé, transactions ACID, requêtes \textbf{paramétrées} $\Rightarrow$ anti-injections SQL
      \item \tech{SQL vs NoSQL} : besoin fondamentalement relationnel (FK, intégrité stock/commande) $\Rightarrow$ pas de NoSQL au MVP ; \textbf{Redis} identifié comme piste si la charge le justifie
      \item \tech{PostFinance} : TWINT natif, sandbox accessible, 100\% suisse \quad\badgeorange{Migration depuis Payrexx}
      \item \tech{GitHub Actions} + \tech{Vercel} : tests $\rightarrow$ déploiement continu, CDN frontend gratuit
    \end{itemize}
  \end{block}
  \vspace{4pt}
  \begin{exampleblock}{\small Architecture en couches, dans le détail}
    \small Chaque router s'appuie sur un \texttt{CrudBase<T>} générique (\texttt{list/get/create/update/delete}) et sur l'injection de dépendances FastAPI (\texttt{Depends()}) --- diagramme à la slide suivante. \textit{J'y reviens en détail dans les difficultés rencontrées.}
  \end{exampleblock}
\end{frame}
...
\begin{frame}{Gestion de projet --- Scrum simplifié \& environnement}
  \begin{columns}[T]
    ...
    \begin{exampleblock}{\small Adapté au projet}
      \small Configuré dès le sprint 1 pour tester et intégrer en continu : \texttt{startapp.sh} lance backend + frontend en une commande.
    \end{exampleblock}
    \vspace{5pt}
    \begin{center}
...
% ─────────────────────────────────────────
%  SLIDE 10 — PIVOT : LE LIVRE TÉMOIN
% ─────────────────────────────────────────
\begin{frame}{Notre fil rouge --- un exemplaire, une trajectoire complète}
  \begin{columns}[T]
    \column{0.5\textwidth}
    \begin{exampleblock}{Le livre que nous allons suivre}
      \small \textit{L'Étranger}, Albert Camus (Folio) --- ISBN \texttt{978-2-07-036002-4}.
      Déposé en rayon par un donateur, il traîne depuis des semaines chez Caritas.
      Nous allons le suivre jusqu'à sa vente.
    \end{exampleblock}
    \vspace{6pt}
    ...
    \end{center}
    \vspace{4pt}
    \badgeorange{Démo live possible à partir d'ici}
  \end{columns}
\end{frame}

% ─────────────────────────────────────────
%  SLIDE 11 — AUTHENTIFICATION (PRÉREQUIS)
% ─────────────────────────────────────────
\begin{frame}{Avant toute chose --- le bénévole doit se connecter}
  \begin{columns}[T]
    ...
    \begin{alertblock}{\small Sécurité de démarrage}
      \small L'API \textbf{refuse de démarrer} si \texttt{SECRET\_KEY} vaut une valeur
      faible/placeholder connue ; si absente, une clé aléatoire est générée par processus.
    \end{alertblock}
    \column{0.46\textwidth}
    \begin{block}{Connexion via Google --- second flux}
      \begin{itemize}\small
        \item \texttt{POST /auth/google} : le jeton est revérifié \textbf{côté serveur} auprès de Google, jamais de confiance aveugle envers le frontend
        \item Compte retrouvé/lié par \texttt{google\_id}, sinon créé
      \end{itemize}
    \end{block}
    \vspace{4pt}
    \begin{alertblock}{\small Contrôle critique}
      \small Si \texttt{email\_verified} = \textit{false} côté Google, connexion
      \textbf{refusée (400)} --- sinon un compte à email non vérifié permettrait une
...
% ─────────────────────────────────────────
%  SLIDE 12 — ÉTAPE 1 : SCAN ISBN (CODE)
% ─────────────────────────────────────────
\begin{frame}[fragile]{Étape 1 --- Le bénévole scanne notre \textit{Étranger}}
...
\begin{frame}{Étape 1 --- Maquette Scan ISBN (mobile)}
  ...
  \begin{tikzpicture}
    ...
    \node[font=\tiny] at (2.6,4.85) {Titre : \textit{L'Étranger (prérempli)}};
    \fill[mybluepale] (1.1,4.0) rectangle (4.1,4.5);
    \node[font=\tiny] at (2.6,4.25) {Auteur : \textit{Albert Camus (prérempli)}};
  ...
  \end{tikzpicture}}\\[4pt]
  \scriptsize\textcolor{mygray}{RGAA : libellés explicites (pas de placeholder seul), bouton $\geq$ 44px, alternative de saisie manuelle}
  \end{center}
\end{frame}

% ─────────────────────────────────────────
%  SLIDE 13 — ÉTAPE 2 : CATALOGUE
% ─────────────────────────────────────────
\begin{frame}{Étape 2 --- Notre livre apparaît au catalogue}
  ...
  \begin{tikzpicture}
    ...
    % Annotation flèche sur Camus
    \draw[myaccent,thick,-{Stealth[length=2mm]}] (1.15,1.7) -- (1.15,2.05);
    \node[font=\tiny\bfseries,text=myaccent] at (1.15,1.5) {notre livre};
  \end{tikzpicture}}\\[4pt]
  \scriptsize\textcolor{mygray}{RGAA : contraste $\geq$ 4.5:1 sur les prix, zones cliquables $\geq$ 44px, focus clavier cohérent}
  \end{center}
\end{frame}

% ─────────────────────────────────────────
%  SLIDE 14 — ÉTAPE 3 : PAIEMENT (TEXTE)
% ─────────────────────────────────────────
...
\begin{frame}{Démonstration en direct --- l'achat de notre livre}
  \begin{alertblock}{\small Passage à l'application réelle}
    \small Je bascule maintenant sur \texttt{caribooks.vercel.app} pour acheter
    \textit{L'Étranger} en sandbox PostFinance, en direct.
  \end{alertblock}
  \vspace{6pt}
  \begin{columns}[T]
  ...
  \vspace{4pt}
  \begin{center}
  \scriptsize\textcolor{mygray}{Plan B --- ces captures d'écran servent de secours si la démo en direct n'est pas possible}
  \end{center}
\end{frame}

% ─────────────────────────────────────────
%  SLIDE 16 — ÉTAPE 4 : LIVRAISON ET STOCK
% ─────────────────────────────────────────
\begin{frame}[fragile]{Étape 4 --- Réserver le stock et choisir la livraison}
  ...
  \begin{columns}[T]
    ...
    \begin{exampleblock}{\small Ce qui m'a fait le plus peur}
      \small Deux clients qui achètent le \textbf{dernier} exemplaire de notre livre
      au même instant. Sans verrou, les deux transactions liraient \og en stock \fg{}
      et vendraient le même livre deux fois.
    \end{exampleblock}
...
\end{lstlisting}
    \begin{exampleblock}{\small Pourquoi \texttt{FOR UPDATE} ?}
      \small La seconde transaction attend la première puis relit un
...
% ─────────────────────────────────────────
%  SLIDE 17 — ÉTAPE 5 : BACK-OFFICE ADMIN
% ─────────────────────────────────────────
\begin{frame}{Étape 5 --- L'association clôture la commande}
  ...
  \begin{columns}[T]
    ...
    \begin{exampleblock}{\small Test d'isolation dédié}
      \small \texttt{test\_orders\_cross\_user\_isolation} : un client ne peut ni lire
      ni modifier les commandes d'un autre (\textbf{404}) ; un non-admin reçoit
      \textbf{403} sur \texttt{/orders/admin/*}.
    \end{exampleblock}
...
% ─────────────────────────────────────────
%  SLIDE 18 — DIFFICULTÉS RENCONTRÉES
% ─────────────────────────────────────────
\begin{frame}{Difficultés rencontrées --- ce que j'ai appris en le faisant seul}
  \begin{itemize}\small
    \item \textbf{Changement de technologie backend} : première version en Express.js,
      abandonnée pour FastAPI face aux exigences de robustesse et de validation
      automatique --- coûteux en temps, mais gain net en productivité.
    \item \textbf{Performance du scan de code-barres} : latence et taux de reconnaissance
      insuffisants en première implémentation ; j'ai dû revoir l'algorithme de détection.
    \item \textbf{Changement de prestataire de paiement} : Payrexx envisagé puis remplacé
      par PostFinance, dont la sandbox ne nécessite pas de carte bancaire réelle.
    \item \textbf{Gestion du périmètre} : la Product Owner informelle voulait régulièrement
      ajouter des fonctionnalités (favoris, chat) ; j'ai dû tenir le MVP et reporter au backlog.
    \item \textbf{Concilier formation et projet personnel} : développement uniquement en
      soirée, 3 jours/semaine, en parallèle de l'alternance.
  \end{itemize}
  \vspace{6pt}
  \begin{alertblock}{Une question pour vous}
    \small Face à une commanditaire qui redemande sans cesse de nouvelles fonctionnalités,
    qu'auriez-vous fait à ma place ?
    \pause \\[3pt]
    \textit{Voici comment j'ai tranché : chaque demande passait par le backlog, jamais
    par un ajout direct au sprint en cours --- le MVP restait la seule priorité contractuelle.}
  \end{alertblock}
\end{frame}
...
%  SLIDE 18 — BLOCS CDA
% ─────────────────────────────────────────
\begin{frame}{Couverture des 3 blocs de compétences CDA --- traçabilité}
  \begin{center}
  \small
  \renewcommand{\arraystretch}{1.5}
  \begin{tabular}{@{}p{2.6cm}p{5.4cm}p{3.6cm}@{}}
    \toprule
    \textbf{Bloc} & \textbf{Compétences clés} & \textbf{Déjà montré à~:} \\
    \midrule
    \textbf{1} --- Sécuriser
      & JWT + bcrypt + Google OAuth, ORM paramétré, Pydantic, RGPD/nLPD
      & Authentification, Sécurité OWASP, RGPD \& accessibilité \\
    \textbf{2} --- Concevoir \& développer
      & MCD/MLD (13 tables), architecture en couches, API REST, intégrations tierces
      & Modèle de données, Architecture, étapes Scan / Paiement \\
    \textbf{3} --- Préparer le déploiement
      & CI/CD GitHub Actions, rolling deploy, 99/99 tests
      & Tests et couverture, Déploiement \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{6pt}
  \begin{exampleblock}{\small Pas de redite}
    \small Chaque ligne renvoie à ce que vous avez déjà vu en action --- le but ici
    est juste de rendre explicite le lien avec le référentiel, pas de tout réexpliquer.
  \end{exampleblock}
\end{frame}
...
\begin{frame}{Conclusion --- Bilan et perspectives}
  \begin{columns}[T]
    ...
    \begin{exampleblock}{\small Et notre livre, dans tout ça ?}
      \small \textit{L'Étranger} a fait le tour complet du système --- scanné, catalogué,
      acheté, payé, livré --- exactement le parcours que suivra chacun des livres réels
      déposés chez Caritas.
    \end{exampleblock}
  \end{columns}
\end{frame}
