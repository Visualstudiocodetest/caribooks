...
\begin{frame}{Plan de la présentation}
  \begin{columns}[T]
    ...
    \begin{exampleblock}{Le principe de cette présentation}
      \begin{itemize}\small
        \item Pas une liste de fonctionnalités
        \item \textbf{Un seul livre suivi}, du dépôt chez Caritas à la livraison
        \item À chaque étape : la brique technique qui le fait avancer
      \end{itemize}
    \end{exampleblock}
    \vspace{6pt}
    \begin{center}
...
\begin{frame}{Contexte du projet — Association Caritas}
  ...
  \begin{exampleblock}{\small Pourquoi j'ai choisi ce projet}
    \begin{itemize}\small
      ...
      \item Un périmètre complet porté seul : de la base de données au serveur de production
    \end{itemize}
  \end{exampleblock}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{alertblock}{Lacunes des solutions existantes}
      \begin{itemize}\small
        ...
        \item Abonnements mensuels incompatibles avec une association
      \end{itemize}
    \end{alertblock}
    \column{0.48\textwidth}
...
\begin{frame}{Stack technique --- Les choix, et pourquoi je les ai faits}
  ...
  \end{block}
  \vspace{4pt}
  \begin{exampleblock}{\small Architecture en couches --- vue d'ensemble}
    \begin{itemize}\small
      \item \texttt{CrudBase<T>} générique par router
      \item Injection de dépendances FastAPI (\texttt{Depends()})
      \item Diagramme détaillé à la slide suivante
    \end{itemize}
  \end{exampleblock}
\end{frame}
...
\begin{frame}{Étape 2 --- Notre livre apparaît au catalogue}
  ...
  \begin{tikzpicture}
    ...
    % Annotation flèche sur notre livre
...
\begin{frame}{Démonstration en direct --- l'achat de notre livre}
  \begin{alertblock}{\small Passage à l'application réelle}
    \begin{itemize}\small
      \item Bascule sur \texttt{caribooks.vercel.app}, sur mon téléphone
      \item Achat de \textit{Divergente T.2} en sandbox PostFinance, en direct
    \end{itemize}
  \end{alertblock}
  \vspace{6pt}
  \begin{columns}[T]
  ...
  \vspace{4pt}
  \begin{itemize}\scriptsize
    \item[] \textcolor{mygray}{\textbf{Plan B} : captures d'écran de secours}
  \end{itemize}
\end{frame}
...
\begin{frame}{Couverture des 3 blocs de compétences CDA --- traçabilité}
  ...
  \begin{tabular}{@{}p{4cm}p{6cm}@{}}
    \toprule
    \textbf{Bloc} & \textbf{Déjà montré à~:} \\
    \midrule
    \textbf{1} --- Sécuriser & Authentification, Sécurité OWASP, RGPD \& accessibilité \\
    \textbf{2} --- Concevoir \& développer & Modèle de données, Architecture, étapes Scan / Paiement \\
    \textbf{3} --- Préparer le déploiement & Tests et couverture, Déploiement \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{8pt}
  \begin{exampleblock}{\small Pas de redite}
    \begin{itemize}\small
      \item Chaque ligne renvoie à ce qui a déjà été montré en action
      \item Objectif : expliciter le lien avec le référentiel
    \end{itemize}
  \end{exampleblock}
\end{frame}
