...
\begin{frame}{Contexte du projet --- Diagramme et besoins}
  \begin{columns}[T]
    ...
    \begin{alertblock}{\small Trois profils métier, mais combien de rôles techniques ?}
      \begin{itemize}\small
        \item \textbf{Bénévoles Caritas} --- ajout rapide de stock \textit{(rôle \texttt{admin})}
        \item \textbf{Administrateur} --- modération et suivi \textit{(rôle \texttt{admin})}
        \item \textbf{Acheteurs} --- trouver un livre pas cher en CHF \textit{(rôle \texttt{user})}
        \item[$\Rightarrow$] \textbf{2 rôles seulement} : bénévole et administrateur partagent le même compte \texttt{admin}
      \end{itemize}
    \end{alertblock}
    \vspace{6pt}
...
\section{Solutions existantes et choix technique}
...
def get_isbn_metadata(isbn: str):
    ...
    if r.status_code == 200:
        ...
        if edition.get("title"):
            return build_book(edition)  # "Divergente - tome 2", Roth...
...
\begin{frame}{Étape 1 --- Flux d'ajout ($<$ 2 min)}
  ...
  \begin{tabular}{c}
  \includegraphics{diagrams/activites_isbn_slide1.pdf}\\[3mm]
  \includegraphics{diagrams/activites_isbn_slide2.pdf}
  \end{tabular}}
  \end{center}
\end{frame}
...
def _lock_stock_rows(db, id_livre):
    return (
  ...
  \begin{columns}[T]
    ...
    \begin{alertblock}{\small Qu'arrive-t-il si un client consulte la commande d'un autre ?}
      \begin{itemize}\small
        ...
        \item Non-admin $\rightarrow$ \texttt{/orders/admin/*} : \textbf{403}      \end{itemize}
    \end{alertblock}
    \column{0.54\textwidth}
    \begin{center}
...
\begin{frame}{Difficultés rencontrées \& Cybersécurité --- OWASP Top 10}
  \begin{columns}[T]
    \column{0.46\textwidth}
    \begin{block}{\scriptsize Ce que j'ai appris en le faisant seul}
      \begin{itemize}\scriptsize
        \item \textbf{Backend Express.js $\rightarrow$ FastAPI} --- robustesse et validation automatique
        \item \textbf{Scan code-barres} --- latence/reconnaissance insuffisantes, algorithme revu
        \item \textbf{Paiement Payrexx $\rightarrow$ PostFinance} --- sandbox sans CB réelle
        \item \textbf{Gestion du périmètre} --- demandes récurrentes (favoris, chat) au backlog
        \item \textbf{Formation + projet perso} --- développement en soirée, 3 j/semaine
        \item \textbf{Vercel + \texttt{output: standalone}} --- assets 404 en prod, corrigé
      \end{itemize}
    \end{block}
    \vspace{4pt}
    \begin{exampleblock}{\scriptsize Gestion du commanditaire}
      \scriptsize
      Chaque nouvelle demande $\rightarrow$ backlog, jamais le travail en cours : le MVP
      restait la seule priorité contractuelle.
    \end{exampleblock}
    \column{0.52\textwidth}
    \tiny
    ...
    \begin{tabular}{@{}p{2.5cm}p{3.0cm}@{}}
      \toprule
      \textbf{Risque} & \textbf{Mesure / Techno} \\
      \midrule
      Injection SQL               & SQLAlchemy ORM (paramétré) \\
      Mots de passe compromis     & bcrypt / pwdlib (cost=12) \\
      Accès non autorisé          & JWT + HTTPBearer \\
      Élévation de privilèges     & \texttt{require\_admin} FastAPI \\
      Données invalides           & Pydantic + HTTPException \\
      Token intercepté            & JWT \texttt{exp} + TLS prod \\
      Secrets en clair            & \texttt{.env} + python-dotenv \\
      Race condition stock        & \texttt{with\_for\_update()} SQL \\
      Non-conformité RGPD/nLPD    & \texttt{/users/me}, bannière cookies \\
      Montant PostFinance falsifié & \texttt{amount\_matches\_commande} \\
      \bottomrule
    \end{tabular}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{block}{Protection des données — RGPD / nLPD}
      \begin{itemize}\small
        ...
        \item \textbf{Effacement} : \texttt{DELETE /users/me} --- anonymisation, commandes conservées, refusé (409) si commande en cours
      \end{itemize}
    \end{block}
    \column{0.5\textwidth}
...
\begin{frame}{Tests et couverture --- 101/101, 78\,\%}
  \begin{columns}[T]
    ...
    \begin{alertblock}{\small Axes de renforcement (Phase 2)}
      \begin{itemize}\small
        \item \texttt{image\_service} (41\,\%)
        \item \texttt{order\_admin\_router} (38\,\%)
        \item \texttt{user\_router} (47\,\%)
      \end{itemize}
    \end{alertblock}
    \column{0.50\textwidth}
    \begin{center}
