  ...
  \begin{columns}[T]
    ...
    \begin{block}{Pipeline CI/CD --- GitHub Actions}
      \begin{enumerate}\small
        ...
        \item Deploy sur \textbf{Oracle Cloud} (2 VMs AMD)
    ...
    \begin{block}{Backend \& API --- Oracle Cloud (Always Free)}
      \begin{itemize}\small
        \item \textbf{2× VM AMD Micro} (Ubuntu 22.04) : API \textbf{FastAPI} via \textbf{Uvicorn + Nginx} (\texttt{:8000})
        \item \textbf{OCI Load Balancer} + sonde \texttt{/health} $\Rightarrow$ haute dispo
        \item Service \textbf{systemd} (redémarrage auto), conteneurs \textbf{Docker}
        \item \textbf{MySQL HeatWave} en sous-réseau \textbf{privé} (VCN, port 3306 fermé)
